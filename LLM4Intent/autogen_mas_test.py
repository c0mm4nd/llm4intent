...
def start_agents():
    ...
    #             "api_key": os.environ.get("OPENAI_API_KEY"),  # 使用OpenAI的API密钥
    #         },
    #     ],
    #     "temperature": 0.1,
    # }
    ...
    user_proxy = UserProxyAgent(
        ...
        system_message="Execute commands and return results directly. No small talk."
    ...
    basic_transaction_retriever = AssistantAgent(
        ...
        llm_config=retrieve_llm_config,
        system_message="Retrieve only essential transaction data. No explanations needed."
    ...
    intent_analyzer = AssistantAgent(
        ...
        system_message="Analyze transaction intent and respond with one word only."
        # system_message="You are a helpful AI assistant. "
    ...
    register_function(
        ...
        description="Get the transaction details from a given transaction hash",
    )
    result = user_proxy.initiate_chat(
        manager,
        message=f"Analyze the intent behind transction with hash {test_transaction_hash} by data from basic transaction retriever and summarize in one-word",
        max_turns=3,
