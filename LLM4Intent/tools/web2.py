...
def search_webpage_from_google(query):
...
def extract_webpage_info_by_urls(urls: List[str]):
...
def get_contract_ABI_from_whatsabi(contract_address: str) -> str:
    url = f"https://evmlookup.web3resear.ch/api/whatsabi?contract={contract_address}"
    return Web3.to_json(requests.get(url).json())
